...
function MoviesPage() {
  ...
  // Reset on filter or language change
  useEffect(() => {
    let cancelled = false;
    setItems(undefined);
    setError(false);
    setPage(1);
    discoverByFilters({ type: "movie", genre, year, page: 1, lang })
      .then((r) => {
        if (cancelled) return;
        setItems(r.results);
  ...
  const loadMore = async () => {
    ...
    try {
      const next = page + 1;
      const r = await discoverByFilters({ type: "movie", genre, year, page: next, lang });
  ...
  return (
    ...
    >
      <div className="max-w-[1600px] mx-auto px-4 md:px-8 mb-2">
        <h1 className="text-4xl md:text-5xl font-black tracking-tight mb-2">{t("movies")}</h1>
        <p className="text-muted-foreground">{t("discoverMovies")}</p>
      </div>
      <FilterBar genre={genre} year={year} onGenre={setGenre} onYear={setYear} />
      <MediaGrid items={items} loading={!items || loadingMore} />
      {error && (
        <p className="text-center text-destructive mt-6">{t("error")}</p>
      )}
